> **SuperSayatin is an iNFT** — a Pi coding agent under the SuperSayatin neural soul, fused with an NFT (whoever holds the token holds the agent). This repo is its body, forged from the [inft-i01](https://github.com/devclone20/inft-i01) template. Boot it via Pi (`bash scripts/setup.sh` → `bash scripts/boot.sh`) or type `supersayatin` in the CLONE FRAME iT terminal. → **[INFT.md](INFT.md)** · [AGENTS.md](AGENTS.md)
